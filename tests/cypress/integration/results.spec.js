 ...
 */
 ...
 const config = {
	pages: [ // pages to test (recommended: 1 search page, 1 category page)
	...
	selectors: {
		...
		sortBy: {
			native: 'select#SortBy', // sort by <select> element (if applicable)
		},
		pagination: {
			prev: '.ss__pagination__page.ss__pagination__page--previous',	// pagination previous
			page: '.ss__pagination__page', 									// pagination page
			next: '.ss__pagination__page.ss__pagination__page--next', 		// pagination next
		},
		results: {
			productWrapper: '.ss__result', // result wrapper
		},
	},
};
...
config?.pages?.forEach((page, _i) => {
	if(page?.url) {
		describe(`${page.id || _i}`, () => {
			...
			if (_i === 0) { // only take screenshot for first page
				describe('Snapshot', () => {
